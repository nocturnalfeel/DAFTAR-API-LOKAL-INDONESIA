...
| PUBG API | PUBG | [Link](https://documentation.pubg.com/en/api-keys.html) | `Aktif` | We require that a JSON Web Token JWT be sent along with requests via the Authorization header. There’s no need to create JWTs manually since they will be created for you when you register for the API - Register at https://developer.pubg.com to receive a free API key with a rate limit of 10 requests per minute. You can find more information about rate limits on the Rate Limits page. | `Auth` |
## Kontak
